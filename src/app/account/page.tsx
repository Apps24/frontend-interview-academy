...
export default async function AccountPage() {
  ...
  const [{ data: profile }, { data: progressRows }, { count: attempts }, { count: sprintCompleted }, { count: bookmarks }, { data: entitlement }, { count: practiceSolved }, { count: lastMinuteCompleted }, { data: latestDiagnostic }] = await Promise.all([
  ...
  return <main className="app-shell account-shell">
    ...
    <section className="account-grid page-width">
      ...
      <aside className="account-stats"><div><span>Plan</span><strong className="plan-name">{entitlement?.status === "active" ? entitlement.plan : "free"}</strong></div><div><span>Latest diagnostic</span><strong>{diagnosticScore === null ? "—" : `${diagnosticScore}%`}</strong></div><div><span>Coding problems</span><strong>{practiceSolved ?? 0}/6</strong></div><div><span>Last-minute blocks</span><strong>{lastMinuteCompleted ?? 0}</strong></div><div><span>Saved questions</span><strong>{bookmarks ?? 0}</strong></div><div><span>Quiz attempts</span><strong>{attempts ?? 0}</strong></div><div><span>Lessons completed</span><strong>{completedLessons}/{javascriptLessons.length}</strong></div><div><span>Sprint completed</span><strong>{sprintCompleted ?? 0}/{sprintItems.length}</strong></div><Link href="/diagnostic">Retake diagnostic →</Link><Link href="/last-minute">Open last-minute plan →</Link><Link href="/practice">Continue coding practice →</Link><Link href="/interview">Review interview questions →</Link><Link href="/sprint">Open two-day sprint →</Link></aside>
    </section>
  </main>;
}
