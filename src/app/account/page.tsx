...
export default async function AccountPage() {
  ...
  const [{ data: profile }, { data: progressRows }, { count: attempts }, { count: sprintCompleted }] = await Promise.all([
  ...
  return <main className="app-shell account-shell">
    ...
    <section className="account-grid page-width">
      ...
      <aside className="account-stats"><div><span>Quiz attempts</span><strong>{attempts ?? 0}</strong></div><div><span>Lessons completed</span><strong>{completedLessons}/{javascriptLessons.length}</strong></div><div><span>Sprint completed</span><strong>{sprintCompleted ?? 0}/{sprintItems.length}</strong></div><Link href="/sprint">Open two-day sprint →</Link></aside>
    </section>
  </main>;
}
