...
export default async function InterviewQuestionPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [{ data: answer }, { data: bookmark }, { data: entitlement }] = await Promise.all([
    supabase.from("interview_question_answers").select("detailed_answer, code_example, follow_ups").eq("question_id", question.id).maybeSingle(),
  ...
  return <main className="app-shell answer-page">
    ...
    <article className="answer-layout page-width">
      ...
      <section className="short-answer"><span className="overline accent">30-SECOND ANSWER</span><p>{question.short_answer}</p></section>
      {answer ? <>
