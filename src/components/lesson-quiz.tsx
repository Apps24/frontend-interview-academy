...
import { completeLesson, submitQuizAnswer } from "@/app/learn/javascript/variables-and-types/actions";
...
export function LessonQuiz({ question, isAuthenticated, initialProgress }: LessonQuizProps) {
  ...
  function checkAnswer() {
    ...
    startTransition(async () => {
      const result = await submitQuizAnswer(selected);
  ...
  <div className="lesson-footer-nav"><Link href="/" className="button button-secondary">← Dashboard</Link><button className="button button-primary" type="button" disabled={isPending || (progress === 0 && !submitted)} onClick={finishLesson}>{progress === 100 ? "Completed ✓" : "Complete & continue →"}</button></div></>;
}
